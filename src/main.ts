...
const impact = [
  {
    label: "100+ reports",
    text: "Generated more than 100 multi-source intelligence reports and situational briefs annually."
  },
  {
    label: "50% faster",
    text: "Improved intelligence dissemination by building automated Excel-based data workflows."
  },
  {
    label: "11+ platforms",
    text: "Built situational dashboards by integrating data from more than 11 secure intelligence systems."
  }
];

const certifications = [
  {
    title: "Publicly Available Information Research (PAIR+)",
    issuer: "U.S. Army Pacific",
    date: "August 2024"
  },
  {
    title: "Open-Source Intelligence Basic Course (OBC)",
    issuer: "U.S. Army Pacific",
    date: "January 2024"
  },
  {
    title: "Cyber Technical Training for Intelligence Professionals (CTTIP)",
    issuer: "U.S. Army Network Enterprise Technology Command",
    date: "October 2022"
  }
];
                ...
                .map(
                  (item) => `
                    <article class="rounded-2xl border border-line/80 bg-white p-5 shadow-panel">
                      <p class="text-sm font-semibold uppercase tracking-[0.18em] text-brass">${item.label}</p>
                      <p class="mt-3 text-sm leading-7 text-slate">${item.text}</p>
                    </article>
                  `
                )
                .join("")}
          ...
          <div class="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ${expertise
              .map(
                (item) => `
                  <article class="rounded-2xl border border-line/80 bg-white p-5 shadow-panel">
                    <p class="text-base font-semibold text-ink">${item}</p>
                  </article>
                `
              )
              .join("")}
            ...
            <div class="grid gap-5 md:grid-cols-3">
              ${certifications
                .map(
                  (item) => `
                    <article class="rounded-2xl border border-line/80 bg-white p-6 shadow-panel">
                      <p class="text-sm font-semibold uppercase tracking-[0.16em] text-field">${item.date}</p>
                      <h3 class="mt-3 text-lg font-semibold text-ink">${item.title}</h3>
                      <p class="mt-3 text-sm leading-7 text-slate">${item.issuer}</p>
                    </article>
                  `
                )
                .join("")}
